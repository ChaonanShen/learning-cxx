...
bool is_fibonacci(int *arr, int len, int stride) {
    ASSERT(len >= 3, "`len` should be at least 3");
    // TODO: 编写代码判断从 arr 开始，每 stride 个元素取 1 个元素，组成长度为 len 的数列是否满足
    // 得搞清楚len究竟是什么，len不是指arr数组长度，而是指{arr, arr+stride, arr+2*stride, ...}这样len个组成数列
    // 所以下面的i仅仅指逻辑上这个每隔stride的数组的位置查找
    for (int i=2; i<len; i++)
        if (arr[i*stride] != arr[(i-2)*stride] + arr[(i-1)*stride])
            return false;
    return true;
}
